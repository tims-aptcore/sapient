...
// Portable equivalents assuming unicode
#ifdef __unix__
#define _countof(x) sizeof(x)
#define sprintf_s snprintf
#define wcstombs_s(r,d,w,s,c) wcstombs(d,s,c)
#endif
...
bool Writer::writeAttribute(const char *strAttribute, std::string strValue)
{
	return writeAttribute(strAttribute, strValue.c_str());
}
...
bool Writer::writeEndElement()
{
	if (_stack.size())
	{
		...
		// Send the message when the root element is closed
		if (_stack.size() == 0)
			return _pStream->Send( true );
...
bool Writer::writeStringElement(const char *strElement, std::string strValue)
{
	return writeStringElement(strElement, strValue.c_str());
}

bool Writer::open(IOutputStream *pStream)
{
	// Don't include the encoding in this implementation
	const char strPreamble[] = "<?xml version=\"1.0\"?>";
